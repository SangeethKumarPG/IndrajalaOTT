...
const HomePage = () => {
  ...
  const scrollFeaturedMovie = (direction) => {
    if (direction === 'left') {
      setCurrentFeaturedMovie((prev) => (prev === 0 ? topFiveMovies.length - 1 : prev - 1));
    } else {
      setCurrentFeaturedMovie((prev) => (prev === topFiveMovies.length - 1 ? 0 : prev + 1));
    }
  };
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentFeaturedMovie((prevIndex) => (prevIndex + 1) % topFiveMovies.length);
    ...
    <>
      ...
      <AppContainer>
        ...
        <MainContent isNavExpanded={isNavExpanded}>
          <ContentWrapper>
            <FeaturedMovieContainer style={screenWidth < 500 ? {width:'350px'}: {width:'100%'}}>
              <ScrollButton 
 ...
 {/*-------------------.---- INDIA PAK BORDER NO WAY YOU TOUCH HERE -------------------  */} 
           ...
           {/*-------------- Trending Page Wala Code Snip Start Here  ----- API Is been fetched  */}
            <SectionTitle>
              ...
              <MovieGridContainer onWheel={handleWheel}>
      {trendingMovies.map((movie) => (
        ...
        >
          ...
          <MovieInfo>
            ...
            <MovieRating className="desktop-only">
              ★ {movie.rating}
            </MovieRating>
